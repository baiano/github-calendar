 ...
 */
const GitHubCalendar = function GitHubCalendar (container, username, options) {
        ...
        if (cal.querySelector("include-fragment")) {
            ...
            if (options.global_stats !== false) {
                ...
                let currentStreakInfo = parsed.current_streak
                    ? `${formatDate(parsed.current_streak_range[0])} &ndash; ${formatDate(parsed.current_streak_range[1])}`
                    : parsed.last_contributed
                    ? `Last contributed in ${formatDate(parsed.last_contributed)}.`
                    : "Rock - Hard Place";
                
                let longestStreakInfo = parsed.longest_streak
                    ? `${formatDate(parsed.longest_streak_range[0])} &ndash; ${formatDate(parsed.longest_streak_range[1])}`
                    : parsed.last_contributed
                    ? `Last contributed in ${formatDate(parsed.last_contributed)}.`
                    : "Rock - Hard Place";
                ...
                firstCol.innerHTML = `<span class="text-muted">Contributions in the last year</span>
                    ...
                    <span class="text-muted">${formatDate(addSubtractDate.add(addSubtractDate.subtract(new Date(), 1, "year"), 1, "day"))} &ndash; ${formatDate(new Date())}</span>`;
