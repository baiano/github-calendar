...
import * as parse from "github-calendar-parser";
...
function addTooltips(container) {
    ...
    days.forEach(day => {
        day.addEventListener("mouseenter", (e) => {
            let contribCount = e.target.querySelector("span").textContent.split(" ")[0];
            if (contribCount === "0") {
...
module.exports = function GitHubCalendar(container, username, options) {
    ...
    let fetchCalendar = () => options.getCalendar(username).then(body => {
        ...
        const headerElement = cal.querySelector(".position-relative h2");
        if (headerElement) {
        ...
        if (options.responsive === true) {
            ...
            // // Width property should be set to 100% to fill the entire container
        ...
        if (options.global_stats !== false) {
            let parsed = parse(cal.outerHTML);
            ...
            firstCol.className = "contrib-column contrib-column-first table-column";
            ...
            secondCol.className = "contrib-column table-column";
            ...
            thirdCol.className = "contrib-column table-column";
        ...
        // If options include tooltips, add tooltips listeners to SVG
